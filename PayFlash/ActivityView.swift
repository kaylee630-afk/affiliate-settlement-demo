    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    HStack(spacing: 12) {
                        statBox("$\(Int(store.totalSettled))", "Total Sent", .green)
                        statBox("\(store.txCount)", "Payments", .blue)
                        statBox("\(Set(store.settlements.map(\.affiliateID)).count)", "Contacts", .orange)
                    }
                    ForEach(store.settlements) { tx in
                        ...
                        HStack(spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 1) { Text(tx.name).font(.subheadline).fontWeight(.medium); Text("$\(tx.orderAmount, specifier: "%.2f") · \(tx.time)").font(.caption).foregroundColor(.secondary) }
                            Spacer()
